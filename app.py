...
st.markdown(
    f"""
    <style>
      .stApp {{ background: {BG}; scroll-behavior: smooth; }}
      ...
      /* Sembunyikan logo GitHub / toolbar sumber kode */
      ...
      /* Sembunyikan tombol >> akses sidebar / unggah file */
      ...
      div[data-testid="stPlotlyChart"], .table-wrap {{
        animation: fadeScale .85s cubic-bezier(.16, 1, .3, 1) both;
      }}
      .hero {{
        ...
        animation: fadeScale .9s cubic-bezier(.16, 1, .3, 1) both;
        animation-delay: 0s;
      }}
      ...
      .kpi:hover {{
        ...
        box-shadow: 0 12px 28px rgba(15,39,68,.10);
      }}
      .kpi.target {{ border-left-color: {NAVY}; animation-delay: .08s; }}
      .kpi.shortfall {{ border-left-color: {SOFT_RED}; animation-delay: .22s; }}
      .kpi .label {{
      ...
      div[data-testid="stPlotlyChart"]:hover {{
        box-shadow: 0 10px 26px rgba(15,39,68,.09);
      }}
      .table-wrap {{ animation-delay: .16s; }}
      ...
      @media (prefers-reduced-motion: reduce) {{
        ...
        .kpi:hover, div[data-testid="stPlotlyChart"]:hover {{
          transform: none; box-shadow: 0 4px 14px rgba(15,39,68,.05);
        }}
        .stApp {{ scroll-behavior: auto; }}
      }}
      .badge-demo {{
